...
import { hasModule, MODULES, type ModuleKey } from "@/lib/auth/modules";
...
export async function denyUnless(module: ModuleKey) {
  ...
  const first = MODULES.find((m) => user.modules.includes(m.key));
    ...
    <div className="flex h-full items-center justify-center p-10">
      <div className="max-w-md rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Seu usuário não tem acesso a nenhuma área liberada. Peça ao administrador para liberar em Permissões.
        </p>
      </div>
    </div>
  );
