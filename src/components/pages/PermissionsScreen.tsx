...
import { MODULES, ROLES, DEFAULT_PERMISSIONS, roleLabel, type RoleKey } from "@/lib/auth/modules";
import {
  Page,
  PageHeader,
  Card,
...
const ROLE_HELP: Record<RoleKey, string> = {
  MASTER: "Acesso total, inclusive Plataforma e Permissões.",
  ADMIN: "Gerencia o dia a dia. Você escolhe as áreas abaixo.",
  SELLER: "Atende leads. Se vinculado a um vendedor, vê só os leads dele.",
  PARTNER: "Acesso de parceiro/revenda. Vincule ao parceiro cadastrado.",
};
...
export function PermissionsScreen() {
  ...
  const isMasterForm = form.role === "MASTER";

  return (
    <Page>
      <PageHeader
        title="Permissões"
        description="Quem acessa o painel e o que cada pessoa pode ver."
        actions={
              ...
              <tbody className="divide-y divide-slate-100">
                {users.map((u) => (
                  <tr key={u.id} className="hover:bg-slate-50/60">
                    ...
                    <td className="px-5 py-3.5 text-slate-600">
                      {u.role === "MASTER" ? (
                        ...
                        <div className="flex max-w-xs flex-wrap gap-1">
                          {MODULES.filter((m) => u.permissions.includes(m.key)).map((m) => (
                    ...
                    <td className="px-5 py-3.5 text-slate-600">
                      {u.seller ? `Vendedor: ${u.seller.name}` : u.partner ? `Parceiro: ${u.partner.name}` : "—"}
                    </td>
      ...
      >
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Field label="Perfil" hint={ROLE_HELP[form.role]}>
            ...
            >
              {ROLES.filter((r) => r.key !== "MASTER" || me?.role === "MASTER").map((r) => (
          ...
          {(form.role === "SELLER" || form.role === "ADMIN") && (
          ...
          <div className="md:col-span-2">
            ...
            {isMasterForm ? (
              <p className="rounded-lg bg-slate-50 px-3 py-2 text-sm text-slate-500">O Master acessa todas as áreas.</p>
            ) : (
