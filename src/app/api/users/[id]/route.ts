...
async function otherActiveMasters(id: string) {
  const [row] = await db
    .select({ n: sql<number>`count(*)::int` })
    .from(appUsers)
    .where(and(eq(appUsers.role, "MASTER"), eq(appUsers.active, true), ne(appUsers.id, id)));
  return row?.n ?? 0;
}
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const target = await db.query.appUsers.findFirst({ where: eq(appUsers.id, id) });
  ...
  const losingMaster =
    target.role === "MASTER" && ((v.role && v.role !== "MASTER") || v.active === false);
  if (losingMaster && (await otherActiveMasters(id)) === 0) {
    return NextResponse.json({ error: "Precisa existir pelo menos um Master ativo" }, { status: 400 });
  }
...
export async function DELETE(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (target.role === "MASTER") {
    ...
    if ((await otherActiveMasters(id)) === 0) {
      return NextResponse.json({ error: "Precisa existir pelo menos um Master ativo" }, { status: 400 });
    }
  }
